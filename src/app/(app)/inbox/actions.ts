...
export async function reviewInboxItemFromQueueAction(formData: FormData) {
  ...
  try {
    ...
    redirect(`/inbox?reviewError=${encodeURIComponent(message)}`);
  }

  revalidatePath("/inbox");
  revalidatePath(`/inbox/${inboxItemId}`);
  redirect(`/inbox?reviewed=${encodeURIComponent(status)}&reviewedItemId=${encodeURIComponent(inboxItemId)}`);
}
...
function toReviewErrorMessage(error: unknown) {
  ...
  if (error instanceof CommunityReplyPostError || error instanceof OutreachSendError) {
    return error.message;
  }
